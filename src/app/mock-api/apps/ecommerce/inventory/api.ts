...
import { entrepreneurs as entrepreneursData, brands as brandsData, categories as categoriesData, products as productsData, tags as tagsData, vendors as vendorsData } from 'app/mock-api/apps/ecommerce/inventory/data';
...
export class ECommerceInventoryMockApi
{
    ...
    private _entrepreneurs: any[] = entrepreneursData;
    ...
    registerHandlers(): void
    {
        ...
        this._fuseMockApiService
        .onGet('api/apps/ecommerce/inventory/entrepreneurs', 300)
        .reply(({request}) =>
        {
            // Get available queries
            const search = request.params.get('search');
            ...
            // Clone the entrepreneurs
            let entrepreneurs: any[] | null = cloneDeep(this._entrepreneurs);

            // Sort the entrepreneurs
            if ( sort === 'nombre' || sort === 'documento' || sort === 'telefono' )
            {
            ...
            {
                entrepreneurs.sort((a, b) => order === 'asc' ? a[sort] - b[sort] : b[sort] - a[sort]);
            }

            // If search exists...
            if ( search )
            {
                // Filter the entrepreneurs
                entrepreneurs = entrepreneurs.filter(contact => contact.name && contact.name.toLowerCase().includes(search.toLowerCase()));
            }

            // Paginate - Start
            const entrepreneursLength = entrepreneurs.length;

            // Calculate pagination details
            const begin = page * size;
            const end = Math.min((size * (page + 1)), entrepreneursLength);
            const lastPage = Math.max(Math.ceil(entrepreneursLength / size), 1);
            ...
            {
                // Paginate the results by size
                entrepreneurs = entrepreneurs.slice(begin, end);

                // Prepare the pagination mock-api
                pagination = {
                    length    : entrepreneursLength,
                    size      : size,
                    page      : page,
                    lastPage  : lastPage,
                    startIndex: begin,
